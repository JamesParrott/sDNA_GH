...
Parse data in a Data Tree or GDM (Geometry and Data Mapping), from a specified field, for subsequent colouring and plotting.  Use this component separately from Recolour_Objects to calculate colours with a visible Grasshoper Colour Gradient component.  
Max and Min bounds can be overridden, else they are calculated on the whole data range.  **WARNING!  Parsing is for the purpose of colourisation, e.g. in order to produce the desired result from Recolour_Objects.  Therefore, although the inputted Data is not changed, the Data outputted almost certainly will be changed, so should be assumed to be false.**  After parsing, the legend tags are the definitive reference for what each colour means, not the outputted data values. 
**Data Tree**
A Data Tree connected to `Data`'s first level should contain a branch for each geometric element in the corresponding list (connected to `Geom`).  Each of these highest branches should contain a branch with only two items:  keys and values.  The two nodes of this structure a pair of corresponsing lists; a list of 'keys' or field names, and a list of 'values' corresponding to the actual numerical data items for that field, for that geometric object.  Read_Shp supplies a Data Tree in this required format, as if read from Usertext.  At the time of writing, this the same format produced by reading in Usertext with the third-party Grasshopper plug-in Elefront).
**Field to plot**
...
**Bounds**
The domain this data is parsed against can be customised by setting the options `plot_min`, `plot_max`, shifting it, widening it or narrowing it, e.g. to exclude erroneous outliers.  If `plot_min`, `plot_max` are both numbers and `plot_min` < `plot_max`, they will be used; otherwise the max and min are automatically calculated.  To go back to automatic calculation after an override, choose values that satisfy `plot_min` >= `plot_max`.  To bound the parsed values of data points lower than `plot_min` or higher than `plot_max` set `bound` to True, or set `exclude` to True to exclude them from Data (and their objs from Geom) from the output altogether.  
...
If objects are coloured according only to the midpoint of the bin / class they are in, the parsed data will take far fewer distinct values than the number of polylines in a large network.  
###### Recolour_Objects
